#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import argparse, time, threading, ipaddress
...
class State:
    def __init__(self):
        self.devices={}; self.lock=threading.Lock(); self.refreshing=False; self.firmware_checking=False
        self.last_refresh=None; self.last_fw=None
        self.cfg={"timeout":3,"refresh":15,"mdns_timeout":5,"user":None,"password":None,"devices":[],"network":None,"use_mdns":True}
s=State()
...
def mdns_discover():
    ...
    zc=Zeroconf(); l=Listener(); b=ServiceBrowser(zc,"_shelly._tcp.local.",l)
    time.sleep(s.cfg["mdns_timeout"]); b.cancel(); zc.close(); return l.found
def network_scan(cidr):
    found={}; timeout=min(float(s.cfg["timeout"]),1.5)
    def check(ip):
        try:
    ...
    with ThreadPoolExecutor(max_workers=64) as ex:
        for f in as_completed([ex.submit(check,str(ip)) for ip in ipaddress.IPv4Network(cidr,strict=False).hosts()]):
            ip,d=f.result()
            if ip: found[ip]=d
    return found
def fw_result(cur=None, latest=None, err=None, has=None):
    ...
    return d
def check_firmware(ip, dev=None):
    dev=dev or {}; gen=int(dev.get("generation") or detect_gen(ip) or 1); cur=dev.get("firmware_current") or dev.get("firmware")
    if gen>=2:
        try:
            r=requests.get(f"http://{ip}/rpc/Shelly.CheckForUpdate",timeout=s.cfg["timeout"],auth=auth2())
            if r.status_code!=200: return fw_result(cur,err=f"HTTP {r.status_code}")
            stable=(r.json().get("stable") or {}); latest=stable.get("version") or stable.get("ver") or stable.get("fw_id")
            return fw_result(cur,latest,has=(True if latest and str(latest)!=str(cur) else False))
        except Exception as e: return fw_result(cur,err=str(e))
    ...
    try:
        ...
        return fw_result(cur,latest,has=has)
    except Exception as e: return fw_result(cur,err=str(e))
def query_device(ip):
    g=detect_gen(ip); d={"ip":ip,"generation":g or 1,"online":False}
    try:
        if g>=2:
            ...
            try:
                st=requests.get(f"http://{ip}/rpc/Shelly.GetStatus",timeout=s.cfg["timeout"],auth=auth2()).json(); w=st.get("wifi",{})
                d.update({"wifi_rssi":w.get("rssi"),"switches":[],"total_power_w":0})
                for k,v in st.items():
            ...
            try:
                ...
                d["total_power_w"]=round(sum(x.get("power",0) or 0 for x in st.get("meters",[])),2)
            except Exception: pass
            d["device_name"]=d.get("model")
        d.update(check_firmware(ip,d)); return d
...
def refresh_devices():
    ...
    with ThreadPoolExecutor(max_workers=24) as ex:
        for f in as_completed([ex.submit(query_device,ip) for ip in ips]):
            q=f.result(); res[q["ip"]]=q
    with s.lock: s.devices.update(res); s.last_refresh=now()
    s.refreshing=False
def discover_all():
    found={ip:{"ip":ip} for ip in s.cfg["devices"]}
    if s.cfg["use_mdns"]: found.update(mdns_discover())
    if s.cfg["network"]: found.update(network_scan(s.cfg["network"]))
    with s.lock:
...
def check_all_fw():
    ...
    with ThreadPoolExecutor(max_workers=24) as ex:
        futs={ex.submit(check_firmware,ip,d):ip for ip,d in snap.items()}
        for f in as_completed(futs):
            with s.lock: s.devices.setdefault(futs[f],{"ip":futs[f]}).update(f.result())
    s.last_fw=now(); s.firmware_checking=False
def set_relay(ip,rid,act):
    try:
        if detect_gen(ip)>=2:
            url=f"http://{ip}/rpc/Switch.Toggle?id={rid}" if act=="toggle" else f"http://{ip}/rpc/Switch.Set?id={rid}&on={'true' if act=='on' else 'false'}"
...
@app.route("/")
def index(): return render_template_string(HTML, refresh=s.cfg["refresh"])
@app.get("/api/devices")
def api_devices():
    with s.lock: return jsonify({"devices":list(s.devices.values()),"last_refresh":s.last_refresh,"last_firmware_check":s.last_fw,"refreshing":s.refreshing,"firmware_checking":s.firmware_checking})
@app.get("/api/summary")
def api_summary():
...
def api_refresh(): threading.Thread(target=refresh_devices,daemon=True).start(); return jsonify(ok=True)
@app.post("/api/discover")
def api_discover(): threading.Thread(target=discover_all,daemon=True).start(); return jsonify(ok=True)
@app.post("/api/firmware/check")
def api_fw_all(): threading.Thread(target=check_all_fw,daemon=True).start(); return jsonify(ok=True)
@app.post("/api/device/<ip>/firmware/check")
def api_fw_one(ip):
    with s.lock: dev=s.devices.get(ip,{"ip":ip})
    fw=check_firmware(ip,dev)
...
def main():
    ...
    threading.Thread(target=discover_all,daemon=True).start(); threading.Thread(target=loop,daemon=True).start(); app.run(host=a.host,port=a.port,threaded=True,debug=False)
